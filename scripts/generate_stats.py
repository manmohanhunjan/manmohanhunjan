...
Env:
  ...
  GH_LOGIN      user to summarise (default: manmohanhunjan)
...
LIGHT = dict(
    data="#6e7681", emph="#424a53", dim="#8c959f", rule="#d8dee4", surface="#ffffff"
)
DARK = dict(
    data="#c9d1d9", emph="#f0f6fc", dim="#8b949e", rule="#30363d", surface="#0d1117"
)
...
def fetch(login, token):
    since, until = window()
    body = json.dumps(
        {"query": QUERY, "variables": {"login": login, "from": since, "to": until}}
    ).encode()
...
def head(w, h, font=None):
    return (
        ...
        f'viewBox="0 0 {w} {
            h}" fill="none" font-family="{MONO}">' + style(font=font)
    )
...
def label(x, y, text, size=11, cls="m-f", anchor="start", extra=""):
    ...
    return (
        f'<text x="{x}" y="{y}" class="{cls}" font-size="{size}"{a}'
        f"{extra}>{text}</text>"
    )
...
def draw_stats(s):
    ...
    p = [head(WIDTH, H)]
    p.append(
        f'<g opacity="0">{fade(0.10)}'
        + label(0, 50, s["total"], 52, "e-f", extra=' font-weight="600"')
        + label(0, 72, "contributions in the last year", 12)
        + "</g>"
    )
    for i, (val, lab) in enumerate(
        ...
        p.append(
            f'<g opacity="0">{fade(0.30 + i * 0.12)}'
            + label(WIDTH, 30 + i * 40, val, 19,
                    "e-f", "end", ' font-weight="600"')
            + label(WIDTH, 47 + i * 40, lab, 11, "m-f", "end")
            + "</g>"
        )
    ...
    ex, ey = pts[-1]
    p.append(
        f'<circle cx="{ex - 2:.1f}" cy="{ey:.1f}" r="4.5" class="e-f r" '
...
def draw_langs(s):
    ...
    for gi, (gx, title, data, as_pct) in enumerate(groups):
        ...
        cid = f"rl{gi}"
        clip, cursor = wipe(
            cid, gx + name_w, 20, bar_max, rows * 22, 0.34 + gi * 0.12, 0.95
        )
        p.append(clip)
...
def draw_year(s):
    ...
    p.append(
        ...
        + label(
            pad_l,
            32,
            f"{s['active']} of {sum(len(w)
                                    for w in weeks)} days had a contribution",
            11,
        )
        + "</g>"
    )
    ...
    for r in range(7):
        ...
        delay = 0.30 + r * 0.07
        p.append(
            f'<clipPath id="{cid}"><rect x="{pad_l}" y="{y}" '
    ...
    for r, lab in ((1, "mon"), (3, "wed"), (5, "fri")):
        p.append(label(pad_l - 7, pad_t + r * LH +
                 FS - 0.6, lab, 9, "m-f", "end"))
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "manmohanhunjan")
    ...
    changed = [n for n, svg in files.items() if write(
        os.path.join(out_dir, n), svg)]
    print(
